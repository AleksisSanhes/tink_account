...
dic = {}
page = 0
from_date = "01.03.2023".split('.')  # сюда вводи дату, с какой счет вести
from_date = date(int(from_date[2]), int(from_date[1]), int(from_date[0]))

upto_date = "01.04.2023".split('.')  # сюда вводи дату, с какой счет вести
...
with pdfplumber.open("periods/1.04.2023.pdf") as temp:
    try:
            ...
            for i in first_page:
                ...
                if separated[0][0].isdigit() and len(separated) > 9:  # нужные строки с датами
                    ...
                    if from_date <= current_date and upto_date >= current_date:
                        sign = separated[3]
                        amount = separated[4] if separated[4][-1] != '₽' else separated[4] + separated[5]
